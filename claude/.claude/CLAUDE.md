...
## Architecture
- When working on technical decisions on one-way doors, do not give much weight to development costs
- Impact is measured by reach, not diff size: a one-line change to a shared interface is a change to every implementer and call site
- A design sketch (3–5 sentences: what is touched, where the change lives and why there, what was rejected) is required when a change:
  - (a) touches a contract others depend on — interface, public signature, shared type, schema, wire format, base class
  - (b) adds new files or new module interactions
  - (c) grows past ~50 lines
